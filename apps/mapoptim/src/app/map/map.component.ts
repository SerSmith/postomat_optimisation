...
export class MapComponent implements AfterViewInit {
    ...
    public r: any;
    public fixedChoosen: boolean = false;
    public recomChoosen: boolean = false;
    ...
    onMapReady(point: any, radius: any) {
        ...
        this.heat = (L as any).heatLayer(heatPoint, {
            radius: 20, maxZoom: 7, opacity: 0.5,
            blur: 10
        }).addTo(this.map);
    }
    ...
    public optFilters(event: any): void {
        ...
        if (this.optType === 'Оптимизация на основе солвера') {
            this.optim.optim(filters).subscribe(rec => {
                ...
                this.recomChoosen = true;
                let arr2 = [];
                arr2 = this.recommPoints;
                if (this.fixedPoints.length) {
                    this.fixedChoosen = true;
                    this.fixedPoints.forEach((el: any) => arr2.push(el));
                }
                this.mapGroup(this.map, arr2);
                this.setHeatMap();
                this.setSaveLink();
            });
        } else {
            this.optim.optim2(filters).subscribe(rec => {
                ...
                this.recomChoosen = true;
                let arr = [];
                arr = this.recommPoints;
                if (this.fixedPoints.length) {
                    this.fixedChoosen = true;
                    this.fixedPoints.forEach((el: any) => arr.push(el));
                }
                this.mapGroup(this.map, arr);
                this.setSaveLink();
                this.setHeatMap();
            });
        }
